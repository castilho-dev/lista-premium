...
export default function UnifiedSection() {
  ...
  return (
    <section
      ref={ref}
      className="py-16 sm:py-20 lg:py-24 section-padding relative overflow-hidden bg-white border-y border-neutral-200/60"
    >
      <div className="container-main max-w-xl relative">
        <h2 className="font-serif font-medium text-3xl sm:text-4xl text-neutral-900 text-balance leading-tight text-center mb-12 sm:mb-14 tracking-tight">
          A próxima história é a{' '}
          <span className="bg-gradient-to-r from-[#8C5E33] to-[#B88A56] bg-clip-text text-transparent">sua</span>
        </h2>

        <div className="relative">
          <div
            className="absolute top-5 sm:top-6 bottom-5 left-4 sm:left-5 w-0.5 bg-neutral-200 -translate-x-1/2 rounded-full"
            aria-hidden
          />
          {TIMELINE.map((step, i) => {
            ...
            return (
              <div key={i} className="relative flex gap-4 sm:gap-5 pb-8 sm:pb-10 last:pb-0">
                <div
                  className="relative z-10 flex-shrink-0 w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-gradient-to-br from-[#B88A56] to-[#8C5E33] flex items-center justify-center text-white shadow-md shadow-amber-900/20"
                  aria-hidden
                >
                  <Icon />
                </div>
                <div className="flex-1 min-w-0 pt-0.5">
                  <p className="font-semibold text-base sm:text-[17px] text-[#8C5E33] mb-2">
                    {step.label}
                  </p>
