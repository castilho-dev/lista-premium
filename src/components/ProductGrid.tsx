...
type ProductGridProps = { showLogo?: boolean }

export default function ProductGrid({ showLogo = true }: ProductGridProps) {
  ...
  return (
    <section ref={ref} className="py-14 lg:py-20 bg-white section-padding border-y border-neutral-200/60">
      <div className="container-main">
        <div className="text-center mb-10 fade-in-section">
          {showLogo && (
            <div className="flex flex-col items-center mb-6">
              <Logo size={72} />
              <span className="mt-3 text-[11px] tracking-[0.3em] uppercase text-[#8C5E33] font-medium">
                Lista Premium
              </span>
            </div>
          )}
          <h2 className="font-serif font-medium text-3xl sm:text-4xl text-neutral-900 text-balance tracking-tight">
            ...
            <span className="bg-gradient-to-r from-[#8C5E33] to-[#B88A56] bg-clip-text text-transparent">Encontrará na Lista</span>
          </h2>
            ...
            {products.map((p, i) => (
              ...
              >
                <button
                  ...
                  className="product-card group w-full text-left bg-white rounded-xl sm:rounded-2xl overflow-hidden border border-neutral-200/70 shadow-sm cursor-pointer hover:shadow-xl hover:shadow-amber-900/5 hover:border-[#B88A56]/30 transition-all duration-300"
                >
                  <div className="relative overflow-hidden rounded-t-xl sm:rounded-t-2xl">
                    ...
                    <span className="product-card-badge absolute top-2 left-2 sm:top-3 sm:left-3 bg-[#B88A56] text-white text-[10px] sm:text-xs font-bold px-1.5 py-0.5 sm:px-2.5 sm:py-1 rounded-md sm:rounded-lg shadow-sm transition-all duration-300">
                      -{p.discount}%
                    </span>
                  </div>

                  <div className="p-3 sm:p-5">
                    <h3 className="font-medium text-neutral-800 text-[11px] sm:text-[13px] leading-snug tracking-tight line-clamp-2 min-h-[2.25rem] sm:min-h-[2.5rem] mb-2 sm:mb-4">
                      {p.name}
                    </h3>
                    ...
                    <div className="space-y-1 sm:space-y-1.5">
                      <p className="text-neutral-400 text-[10px] sm:text-xs">
                        ...
                        <span className="text-neutral-400/80 ml-1 sm:ml-1.5">no mercado</span>
                      </p>
                      <div className="flex flex-wrap items-baseline gap-1 sm:gap-2">
                        <span className="text-[#8C5E33] font-bold text-base sm:text-xl tabular-nums tracking-tight">
                          {formatBRL(p.list)}
                        </span>
                        <span className="text-[9px] sm:text-[10px] font-semibold text-[#5C3D22] uppercase tracking-wider bg-[#EADBC5]/60 px-1.5 py-0.5 sm:px-2 rounded">
                          Lista Premium
                        </span>
                      </div>
                    </div>
            ...
            {galleryImages.map((img, i) => (
              ...
              >
                <button
                  ...
                  className="w-full rounded-xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 bg-[#FAF7F2] cursor-pointer border border-neutral-200/50"
                >
                  <img
                    src={img.src}
                    alt={img.alt}
        ...
        <div className="flex justify-center gap-2 mt-6">
          {Array.from({ length: maxIndexGallery + 1 }, (_, i) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-all duration-300 ${
                i === currentGallery ? 'bg-[#B88A56] w-6' : 'bg-neutral-300 hover:bg-neutral-400'
              }`}
        ...
        <div className="fade-in-section mt-12 text-center">
          <p className="text-neutral-600 mb-6 text-lg max-w-2xl mx-auto">
      ...
      {expandedProductIndex != null && (() => {
        ...
        return (
          ...
          >
            ...
            <div
              className="relative bg-white rounded-2xl border border-neutral-200/80 shadow-2xl max-w-md w-full max-h-[90vh] overflow-auto"
              onClick={e => e.stopPropagation()}
            >
              <div className="relative">
                <div className="aspect-square bg-[#FAF7F2] flex items-center justify-center p-6">
                  <img src={p.img} alt={p.name} className="max-h-full max-w-full object-contain rounded-xl" />
                </div>
                <span className="absolute top-3 left-3 bg-[#B88A56] text-white text-xs font-bold px-2.5 py-1 rounded-lg">-{p.discount}%</span>
              </div>
              <div className="p-5 sm:p-6">
                <h3 className="font-medium text-neutral-900 text-base sm:text-lg leading-snug mb-4">{p.name}</h3>
                <p className="text-neutral-400 text-sm mb-2">
                  <span className="line-through tabular-nums">{formatBRL(p.market)}</span>
                  <span className="ml-1.5">no mercado</span>
                </p>
                <div className="flex flex-wrap items-baseline gap-2">
                  <span className="text-[#8C5E33] font-bold text-xl tabular-nums">{formatBRL(p.list)}</span>
                  <span className="text-xs font-semibold text-[#5C3D22] uppercase tracking-wider bg-[#EADBC5]/60 px-2 py-1 rounded">Lista Premium</span>
                </div>
                <p className="mt-3 text-neutral-400 text-xs">{(expandedProductIndex + 1)} / {products.length}</p>
              </div>
            </div>
          </div>
        )
